...
public class LevelManager : MonoBehaviour {
    ...
    void Start () {
        ...
        {//use the project location
            Directory.CreateDirectory(Application.dataPath + "/levels"); // returns a DirectoryInfo object
            Debug.Log(Application.dataPath);
		}
		else
        {//use the user save location
            Directory.CreateDirectory(Application.persistentDataPath + "/levels"); // returns a DirectoryInfo object
    ...
    public void Rehost()
    {
        GameStateManager.instance.setGlobalStateIndex(GameStateManager.STATE_LOBBY);
	...
	{
		return new Vector3(xI*TILE_WIDTH+(yI%2*(TILE_WIDTH/2)), scale, yI*TILE_LENGTH);
        //return new Vector3(xI * TILE_WIDTH + (yI % 2 * (TILE_WIDTH / 2)), 1, yI * TILE_LENGTH);
    }
